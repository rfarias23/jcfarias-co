...
| 2026-09-05 | 008      | (este commit)             | Causa raíz: lockfile sin binarios de `sharp` → imágenes sin optimizar (2 MB). Lock reparado con `npm update sharp`; hero con import estático + blur; JPG 3840→2560 px, 2.0 MB→592 KB. Lighthouse móvil LCP 12.0 s→2.28 s, desktop 2.1 s→0.50 s, CLS 0. Segundo commit: `lib/next-image-types.d.ts` para que `typecheck` pase en CI sin `next-env.d.ts`. **verificada** |

...
- 2026-09-05 — **CI rojo tras el primer commit (run 33955562132):** `typecheck` falló con `TS2307: Cannot find module '@/public/images/soumaya-hero.jpg'`. El import estático necesita las declaraciones de `next/image-types/global`, que solo llegan vía `next-env.d.ts`, archivo ignorado en git y generado por `next build`. Localmente existía de builds anteriores; en un checkout limpio `typecheck` corre antes que `build`. Reproducido moviendo `next-env.d.ts`; corregido con `lib/next-image-types.d.ts` (una línea: `/// <reference types="next/image-types/global" />`, trackeado, dentro del `include` de `tsconfig.json`). Segundo commit de la spec, como en 005 y 012. Archivo fuera de la lista original; se reporta.
